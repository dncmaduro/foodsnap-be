...
export class AdminApplicationController {
  ...
  async getAll(
    ...
    return this.service.getAllApplications({ type, status, searchText });
  }
  ...
  async updateShipperStatus(@Param('id', ParseIntPipe) id: number, @Body('status') status: string) {
    return this.service.updateShipperStatus(id, status);
  }
